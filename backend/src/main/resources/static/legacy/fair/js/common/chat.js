...
function sendMessage() {
  ...
  stompClient.send("/app/chat/post/" + chatData.currentChatNumber, {},
      JSON.stringify({
        'uuid': getCookie("_uuid"),
        'content': message
      }));
...
function handleChatUpdates(message) {
  if (message) {
    ...
    if (chatData.messages.length > 30) {
...
function updateChat() {
  ...
  for (let i = 0; i < chatData.messages.length; i++) {
    ...
    let assholeTag = (message.timesAsshole < infoData.assholeTags.length) ?
        infoData.assholeTags[message.timesAsshole]
        : infoData.assholeTags[infoData.assholeTags.length - 1];
    row.insertCell(0).innerHTML = "[" + message.timeCreated + "] " + assholeTag
        + " - " + message.username + ":";
