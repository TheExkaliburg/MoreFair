...
function handleLadderInit(message) {
  if (message.status === "OK") {
    if (message.content) {
      ...
      if (!ladderData.yourRanker) {
        ladderData.yourRanker = rankerTemplate;
      }
      ...
      ladderData.yourRanker.vinegar = new Decimal(
          ladderData.yourRanker.vinegar);

      ladderData.firstRanker.power = new Decimal(ladderData.firstRanker.power);
      ladderData.firstRanker.points = new Decimal(
          ladderData.firstRanker.points);
...
function buyBias(event) {
  ...
  if (ladderData.yourRanker.points.cmp(
      Decimal.max(ladderData.firstRanker.points,
          infoData.pointsForPromote.mul(ladderData.currentLadder.number)).mul(
          0.8)) >= 0) {
    if (!confirm(
        "You're really close to the top, are you sure, you want to bias.")) {
...
function buyMulti(event) {
  ...
  if (ladderData.yourRanker.points.cmp(
      Decimal.max(ladderData.firstRanker.points,
          ...
          0.8)) >= 0) {
    if (!confirm(
        "You're really close to the top, are you sure, you want to multi.")) {
...
function beAsshole(event) {
  if (ladderData.firstRanker.you && ladderData.rankers.length >= Math.max(
          infoData.minimumPeopleForPromote, ladderData.currentLadder.number)
      && ladderData.firstRanker.points.cmp(
          infoData.pointsForPromote.mul(ladderData.currentLadder.number)) >= 0
...
function buyAutoPromote(event) {
  ...
  if (ladderData.currentLadder.number >= infoData.autoPromoteLadder
      && ladderData.currentLadder.number !== infoData.assholeLadder
      && ladderData.yourRanker.grapes.cmp(
          getAutoPromoteGrapeCost(ladderData.yourRanker.rank)) >= 0) {
...
function handleVinegar(event) {
  ...
  ladderData.rankers.forEach(ranker => {
    if (event.accountId === ranker.accountId) {
      ...
      if (ladderData.firstRanker.you && event.data.success) {
        setTimeout(function () {
          alert(ranker.username + "#" + ranker.accountId + " threw their "
  ...
  ladderData.rankers[0].vinegar = Decimal.max(
      ladderData.rankers[0].vinegar.sub(vinegarThrown), 0);
}
...
function calculateLadder(delta) {
  ladderData.rankers = ladderData.rankers.sort(
      (a, b) => b.points.sub(a.points));
  ...
  for (let i = 0; i < ladderData.rankers.length; i++) {
    ...
    if (ladderData.rankers[i].growing) {
      ...
      ladderData.rankers[i].points = ladderData.rankers[i].points.add(
          ladderData.rankers[i].power.mul(delta).floor());

      // Calculating Vinegar based on Grapes count
      if (ladderData.rankers[i].rank !== 1 && ladderData.rankers[i].you) {
        ladderData.rankers[i].vinegar = ladderData.rankers[i].vinegar.add(
            ladderData.rankers[i].grapes.mul(delta).floor());
      }

      if (ladderData.rankers[i].rank === 1 && ladderData.rankers[i].you
          && isLadderUnlocked()) {
        ladderData.rankers[i].vinegar = ladderData.rankers[i].vinegar.mul(
            Math.pow(0.9975, delta)).floor();
      }
      ...
      for (let j = i - 1; j >= 0; j--) {
        ...
        if (currentRanker.points.cmp(ladderData.rankers[j].points) > 0) {
          ...
          if (ladderData.rankers[j].growing && ladderData.rankers[j].you
              && ladderData.rankers[j].multiplier > 1) {
            ladderData.rankers[j].grapes = ladderData.rankers[j].grapes.add(
                new Decimal(1));
          }
  ...
  if (ladderData.rankers.length >= Math.max(infoData.minimumPeopleForPromote,
      ladderData.currentLadder.number)) {
    let index = ladderData.rankers.length - 1;
    if (ladderData.rankers[index].growing && ladderData.rankers[index].you) {
      ladderData.rankers[index].grapes = ladderData.rankers[index].grapes.add(
...
function calculateStats() {
  ...
  if (ladderData.rankers.length >= Math.max(infoData.minimumPeopleForPromote,
      ladderData.currentLadder.number)) {
    if (ladderData.firstRanker.points.cmp(
        infoData.pointsForPromote.mul(ladderData.currentLadder.number)) >= 0) {
      if (ladderData.currentLadder.number >= infoData.autoPromoteLadder) {
        let leadingRanker = ladderData.firstRanker.you ? ladderData.yourRanker
            : ladderData.firstRanker;
        let pursuingRanker = ladderData.firstRanker.you ? ladderData.rankers[1]
            : ladderData.yourRanker;

        // How many more points does the ranker gain against his pursuer, every Second
        let powerDiff = (leadingRanker.growing ? leadingRanker.power
            : new Decimal(0)).sub(
            pursuingRanker.growing ? pursuingRanker.power : 0);
        // Calculate the needed Point difference, to have f.e. 30seconds of point generation with the difference in power
        let neededPointDiff = powerDiff.mul(
            infoData.manualPromoteWaitTime).abs();

        ladderStats.pointsNeededForManualPromote = Decimal.max(
            (leadingRanker.you ? pursuingRanker : leadingRanker)
            .points.add(neededPointDiff),
            infoData.pointsForPromote.mul(ladderData.currentLadder.number));
      } else {
        ladderStats.pointsNeededForManualPromote = ladderData.firstRanker.you
            ? ladderData.rankers[1].points.add(1)
            : ladderData.firstRanker.points.add(1);
      }
    } else {
      ladderStats.pointsNeededForManualPromote = infoData.pointsForPromote.mul(
          ladderData.currentLadder.number);
    }
...
function updateLadder() {
  ...
  let endRank = ladderAreaIndex + clientData.ladderAreaSize
      + clientData.ladderPadding - 1;
  ...
  for (let i = 0; i < ladderData.rankers.length; i++) {
    ...
    if (ranker.rank === startRank) {
      writeNewRow(body, ladderData.firstRanker);
    }
    if ((ranker.rank > startRank && ranker.rank <= endRank)) {
      writeNewRow(body,
          ranker);
    }
  }
  ...
  for (let i = body.rows.length;
      i < clientData.ladderAreaSize + clientData.ladderPadding * 2; i++) {
  ...
  $('#infoText').html(
      'Sour Grapes: ' + numberFormatter.format(ladderData.yourRanker.grapes)
      + '<br>' + tag1 + 'Vinegar: ' + numberFormatter.format(
          ladderData.yourRanker.vinegar) + tag2);

  $('#usernameLink').html(ladderData.yourRanker.username);

  $('#rankerCount').html("Rankers: " + ladderStats.growingRankerCount + "/"
      + ladderData.rankers.length);
  $('#ladderNumber').html("Ladder # " + ladderData.currentLadder.number);

  $('#manualPromoteText').html("Points needed for "
      + ((ladderData.currentLadder.number === infoData.assholeLadder)
          ? "being an asshole" : "manually promoting")
      + ": " + numberFormatter.format(
          ladderStats.pointsNeededForManualPromote));
...
function writeNewRow(body, ranker) {
  ...
  let assholeTag = (ranker.timesAsshole < infoData.assholeTags.length) ?
      infoData.assholeTags[ranker.timesAsshole]
      : infoData.assholeTags[infoData.assholeTags.length - 1];
  let rank = (ranker.rank === 1 && !ranker.you && ranker.growing
      && ladderData.rankers.length >= Math.max(infoData.minimumPeopleForPromote,
          ladderData.currentLadder.number)
      && ladderData.firstRanker.points.cmp(
          infoData.pointsForPromote.mul(ladderData.currentLadder.number)) >= 0
      && ladderData.yourRanker.vinegar.cmp(getVinegarThrowCost()) >= 0) ?
      '<a href="#" style="text-decoration: none" onclick="throwVinegar(event)">🍇</a>'
      : ranker.rank;
  ...
  row.insertCell(2).innerHTML = numberFormatter.format(ranker.power) +
      ' [+' + ('' + ranker.bias).padStart(2, '0') + ' x' + (''
          + ranker.multiplier).padStart(2, '0') + ']';
...
function showButtons() {
  ...
  $('#biasTooltip').attr('data-bs-original-title',
      numberFormatter.format(biasCost) + ' Points');
  $('#multiTooltip').attr('data-bs-original-title',
      numberFormatter.format(multiCost) + ' Power');
  ...
  if (ladderData.firstRanker.you && ladderData.firstRanker.points.cmp(
      ladderStats.pointsNeededForManualPromote) >= 0) {
  ...
  if (!ladderData.yourRanker.autoPromote && ladderData.currentLadder.number
      >= infoData.autoPromoteLadder
      && ladderData.currentLadder.number !== infoData.assholeLadder) {
    ...
    autoPromoteTooltip.attr('data-bs-original-title',
        numberFormatter.format(autoPromoteCost) + ' Grapes');
  } else {
...
function isLadderUnlocked() {
  if (ladderData.rankers.length <= 0) {
    return false;
  }
  let rankerCount = ladderData.rankers.length;
  if (rankerCount < getRequiredRankerCountToUnlockLadder()) {
    return false;
  }
  return ladderData.firstRanker.points.cmp(getRequiredPointsToUnlockLadder())
      >= 0;
}

function getRequiredRankerCountToUnlockLadder() {
  return Math.max(infoData.minimumPeopleForPromote,
      ladderData.currentLadder.number);
}
