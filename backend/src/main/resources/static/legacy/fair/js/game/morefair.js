...
function connect() {
  ...
  stompClient.connect({}, function (frame) {
    ...
    stompClient.subscribe('/user/queue/info',
        (message) => onInfoReceived(JSON.parse(message.body)),
...
function loadQOLScripts(skipConfirm = false) {
  if (isQolLoaded || (!skipConfirm && !confirm(
      "The QOL scripts are written and maintained by a 3rd-party, they are not guaranteed to work and may cause issues or break things. Do you want to continue loading these?"))) {
    return;
  }
  ...
  // Main QOL Script
  loadAndRunScript(
      "https://raw.githack.com/LynnCinnamon/fair-game-qol/main/fairgame.js");
  // Lynn's Addon
  loadAndRunScript(
      "https://raw.githack.com/LynnCinnamon/Fairgame-Lynns-QOL-Extensions/master/Lynns%20Extension.js");
  isQolLoaded = true;
}
...
function onInfoReceived(message) {
  if (message.content) {
    ...
    infoData.baseVinegarNeededToThrow = new Decimal(
        infoData.baseVinegarNeededToThrow);
    infoData.baseGrapesNeededToAutoPromote = new Decimal(
        infoData.baseGrapesNeededToAutoPromote);

    // Login
    stompClient.subscribe('/user/queue/account/login',
        (message) => onLoginReceived(JSON.parse(message.body)),
        {uuid: getCookie("_uuid")});
    login()
  }
}
...
function onLoginReceived(message) {
  if (message.status === "OK" || message.status === "CREATED") {
    ...
    chatSubscription = stompClient.subscribe(
        '/topic/chat/' + identityData.highestCurrentLadder,
        (message) => handleChatUpdates(JSON.parse(message.body)),
        {uuid: getCookie("_uuid")});
    stompClient.subscribe('/user/queue/chat/',
        (message) => handleChatInit(JSON.parse(message.body)),
        {uuid: getCookie("_uuid")});
    initChat(identityData.highestCurrentLadder);

    // Init Ladder Connection
    ladderSubscription = stompClient.subscribe(
        '/topic/ladder/' + identityData.highestCurrentLadder,
        (message) => handleLadderUpdates(JSON.parse(message.body)),
        {uuid: getCookie("_uuid")});
    stompClient.subscribe('/topic/global/',
        (message) => handleGlobalUpdates(JSON.parse(message.body)),
        {uuid: getCookie("_uuid")});
    stompClient.subscribe('/user/queue/ladder/',
        (message) => handleLadderInit(JSON.parse(message.body)),
        {uuid: getCookie("_uuid")});
    stompClient.subscribe('/user/queue/ladder/updates/',
        (message) => handlePrivateLadderUpdates(JSON.parse(message.body)),
        {uuid: getCookie("_uuid")});
...
function promptNameChange() {
  let newUsername = window.prompt(
      ...
      ladderData.yourRanker.username);
  if (newUsername && newUsername.length > 32) {
    let temp = newUsername.substring(0, 32);
    alert('The maximum number of characters in your username is 32, not '
        + newUsername.length + '!');
    newUsername = temp;
  }

  if (newUsername && newUsername.trim() !== "" && newUsername
      !== ladderData.yourRanker.username) {
...
function getUpgradeCost(level) {
  return new Decimal(
      Math.round(Math.pow(ladderData.currentLadder.number + 1, level)));
}

function getVinegarThrowCost() {
  return new Decimal(infoData.baseVinegarNeededToThrow.mul(
      new Decimal(ladderData.currentLadder.number)));
}

function getAutoPromoteGrapeCost(rank) {
  let minPeople = Math.max(infoData.minimumPeopleForPromote,
