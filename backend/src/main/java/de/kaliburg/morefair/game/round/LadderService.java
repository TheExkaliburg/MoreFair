...
public class LadderService implements ApplicationListener<AccountServiceEvent> {
  ...
  boolean buyBias(Event<LadderEventTypes> event, LadderEntity ladder) {
    try {
      ...
      int currentUpgrade = ranker.getBias();
      if (currentRound.getNumber() == 300) {
        int ranksBehind =
            currentRound.getHighestAssholeCount() - ranker.getAccount().getAssholeCount();
        currentUpgrade -= ranksBehind;
      }

      BigInteger cost = upgradeUtils.buyUpgradeCost(ladder.getScaling(), currentUpgrade,
          ladder.getTypes());
  ...
  boolean buyMulti(Event<LadderEventTypes> event, LadderEntity ladder) {
    try {
      ...
      int currentUpgrade = ranker.getMultiplier();
      if (currentRound.getNumber() == 300) {
        int ranksBehind =
            currentRound.getHighestAssholeCount() - ranker.getAccount().getAssholeCount();
        currentUpgrade -= ranksBehind;
