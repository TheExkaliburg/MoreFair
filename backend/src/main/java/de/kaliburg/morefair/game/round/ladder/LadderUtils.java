...
public class LadderUtils {
  ...
  public static boolean canPromote(LadderEntity ladder, RankerEntity ranker,
      RankerEntity firstRanker) {
    return ranker.getRank() == 1 && ranker.getId().equals(firstRanker.getId())
        && isLadderUnlocked(ladder,
        firstRanker);
  }
  ...
  public static boolean isLadderUnlocked(@NonNull LadderEntity ladder,
      @Nullable RankerEntity firstRanker) {
    ...
    if (firstRanker == null) {
      firstRanker = ladder.getRankers().stream()
          .max(Comparator.comparing(RankerEntity::getPoints))
