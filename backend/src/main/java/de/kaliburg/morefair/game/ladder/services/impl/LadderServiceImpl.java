...
@Service
@Log4j2
public class LadderServiceImpl implements ApplicationListener<AccountServiceEvent>, LadderService {
  ...
  public LadderServiceImpl(RankerService rankerService, LadderRepository ladderRepository,
  ...
  public void loadIntoCache(RoundEntity round) {
    ...
    if (ladders.isEmpty()) {
      ladders.add(createCurrentLadder(1));
    }
    currentLadderMap = new HashMap<>();
    ladders.forEach(ladder -> {
      // TODO: This "distinct" shouldn't need to be here, i don't know why it pulls multiple instances of
  ...
  public List<LadderEntity> findAllByRound(RoundEntity round) {
    // FIXME do logic, probably only of current Round
    return List.of();
  }
}
