...
public class WSUtils {
  ...
  public void convertAndSendToUser(SimpMessageHeaderAccessor sha, String dest,
      HttpStatus status) {
    simpMessagingTemplate.convertAndSendToUser(sha.getUser().getName(), dest,
  ...
  public void convertAndSendToUser(UUID uuid, String dest, Object content) {
    simpMessagingTemplate.convertAndSend(dest + uuid.toString(),
        new WSMessageAnswer<>(content));
  }
  ...
  public StompPrincipal convertMessageHeaderAccessorToStompPrincipal(
